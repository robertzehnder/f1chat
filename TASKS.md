# Tasks (plan v1 · second opinion: NOT converged after 3 rounds (last verdict REVISE))

A second, independent Monza 2026 blog post authored by GPT-6 from the frozen evidence packet, published at /blog/monza-2026-gpt6 through the existing verified-figures pipeline. T1 fixes the committed gate's root cause in-repo (deterministic figure writes with JSON-normalized comparison, so NaN→null round-trips don't force rewrites) and adds the recipe loader plus a binding-based strict verifier that ties every plotted point to its packet row's lap and rejects unresolved paths; T4 hardens the gate script; T2 authors the full package atomically; T3 does per-renderer browser QA with visual PNG inspection.
...
- [ ] **T2** Author the GPT-6 Monza post: full analyst/2026_1293_gpt6/ package + pipeline run — _pending_ (after T1, T4) `data` `docs` `ui` `tooling`
    - AC: `bash scripts/gate_gpt6_post.sh` exits 0 in the worktree: all seven files present, packet byte-identical, figures verify under strict mode, draft verifies, style lint passes, 900–1400 words, 3–5 figures, post builds with slug monza-2026-gpt6, ≥3 PNGs, and `git diff main` is clean for all Do-not-touch paths
