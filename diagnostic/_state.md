...
- Slice plans must invoke the test-grading gate via `bash scripts/loop/test_grading_gate.sh` or a narrowed `node --test scripts/tests/<slice>.test.mjs`, not raw `cd web && npm run test:grading`; the wrapper diffs failures against `scripts/loop/state/test_grading_baseline.txt` so pre-existing integration failures (e.g. `driver-fallback.test.mjs` Cases A/B/E and `perf-trace-spans.test.mjs:113` suite-ordering flake) do not auto-REJECT (slice:08-synthesis-payload-cutover).
