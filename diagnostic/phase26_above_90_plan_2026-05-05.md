...
## rev6 changes (codex audit pass 6 applied — 2026-05-05)

Four findings; all addressed.

- **MEDIUM (Section 11 audit ask stale against rev5 generator)** —
  audit ask 1 still told codex to verify a `manifest?` column and
  said 8.4a rows are "filtered out of the final target list", but
  rev5's generator emits `phase26_target` instead AND Section 8.1
  is the full 66-row inventory (8.4a rows are NOT filtered out of
  Section 8.1; only `pursuable_targets` filters them).
  **Fix**: rewrote audit ask 1 to verify the `phase26_target`
  column (not `manifest?`) and the `pursuable_targets` derivation
  step (Section 8.5) producing the 63-row pursuable set. Audit
  ask 6 wording also tightened to distinguish "8.4a in Section
  8.1 inventory" (yes, present) from "8.4a in pursuable_targets"
  (no, filtered).
- **MEDIUM (no-lap-distance ceiling component math doesn't close)**
  — rev5 said "+43" but listed components sum to +41/+42:
  26.0=+13, 26.3a=+2, 26.3d=+22, 26.4=+3, 26.5=+1-2 = +41 to +42.
  **Fix**: ceiling restated using the same conservative/optimistic
  range as Section 1's stream-table sums:
  conservative no-lap-distance = +38 - +7 = **+31** → 101+31 =
  132/167 = 79.0%;
  optimistic no-lap-distance = +50 - +7 = **+43** → 101+43 =
  144/167 = 86.2%.
  The "+43" was the optimistic-end max; rev5's prose cited it
  without flagging the conservative range. Section 10 abort
  threshold now lists both ends explicitly.
- **LOW (pursuable_targets pseudocode schema mismatch)** — rev5's
  pseudocode used `r['id']` (int) but the embedded Section 8.1
  table has `qid` strings like `q2182`. If someone parses the
  markdown table directly, the filter `r['id'] not in
  budget_exception_qids` won't fire.
  **Fix**: pseudocode comment clarifies that the input is the
  raw baseline `results` list (pre-table), where `id` is an int.
  Added a sibling pseudocode block showing the filter against
  the parsed markdown-table format (where qid is a `'q####'`
  string) for callers who work from the table only.
- **LOW (rev3/rev4 labels stale across live sections)** — Section
  8 status, bucket summary, per-stream guarantees, and
  implementation-readiness all carried rev3/rev4 labels even
  though the file is rev6 now.
  **Fix**: swept and updated to rev6 (or removed the explicit rev
  label where the content is genuinely stable across revisions).
...
are *cheap* fixes; if any regressions are not actually fixable in
this stream they convert to budget exceptions in Section 8.4a).
...
**A lift target**: ~14-22 of the 30 26.3d candidates,
...
**A lift target**: subset of the 30 26.3d candidates;
...
**A lift target**: 3 candidates (q2161, q2162, q2166 per
...
**A lift target**: q2023 ships under stream 26.3d (Tyre
...
**A lift target**: lap-N JOIN questions surface during
Stream 26.3d's audit pass; once classified they get the matview-
hint update from this sub-stream.

**Stream 26.4 total**: 3 candidates (the 26.4 bucket from
...
**A lift target**: 1-2 questions (durability — borderline-A
...
**Status**: 2026-05-05 baseline consumed. Section 8.1 below
...
**Bucket summary** (from May-5 baseline):
...
If the optimistic end fails to materialize, the next plan revision chooses one of:
1. Expand a stream's scope (most likely 26.3d via a deeper audit
   bucket pass).
2. Phase 27 rollover for the residual ~5-10 questions.

### 8.3 — Exclusions and inclusions verified against May-5 baseline
...
The derivation operates on **the raw baseline non-A list** (the
output of the Section 8.1 generator script before it formats the
markdown table), where each row's `id` field is an int from
...
# (each `r` has int `id`, string `category`, etc — pre-markdown).
pursuable_targets = [r for r in non_a if r['id'] not in budget_exception_qids]
# rewrite_candidate_qids stay in pursuable_targets but are tagged
# with stream 26.3a. 8.4a rows stay in Section 8.1 for inventory
# completeness; they only drop out of pursuable_targets.
```

If working from the rendered Section 8.1 markdown table (where
the column is `qid` with values like `q2182`), the equivalent
filter is:

```python
budget_exception_qids_str = {"q2182", "q2206", "q2207"}
pursuable_targets = [r for r in markdown_rows if r['qid'] not in budget_exception_qids_str]
```

Both produce the same 63-row pursuable set.
...
(90.4%) headline target by 7 questions; the conservative end is
short by 19. Either way, **shipping Phase 26 without 26.1+26.2
cannot reach the 90% line on its own**.

If the optimistic ceiling (86.2%) is acceptable, expanding 26.3d's
audit-bucket pass (deeper per-question classification) could
claw back another 3-5 A grades, pushing the no-lap-distance
ceiling to roughly 88-89%. The hard 90% line still requires
26.1+26.2 OR a Phase 27 grader-loosening pass.
...
1. **Section 8.1 generation correctness**: re-run the
   ...
   - The `phase26_target` column shows A for every pursuable row
     ...
     q2207=C).
   - **Section 8.1 is the full 66-row inventory and INCLUDES the 3
     8.4a budget-exception rows.** Section 8.5 separately derives
     `pursuable_targets` (63 rows) by filtering out 8.4a entries.
     Verify both: 8.1 has 66 rows (including 8.4a); the
     `pursuable_targets` derivation in 8.5 produces 63 rows.
   - Section 8.4b rewrite candidates (q2100, q2144) stay in BOTH
     Section 8.1 AND `pursuable_targets` with stream tag 26.3a.
...
6. **Outcome math closure**: walk through the per-stream
   ...
   - q2100 and q2144 (rewrite candidates) are present in BOTH
     Section 8.1 (full inventory) AND `pursuable_targets`
     (the 63-row filter) with stream tag 26.3a. They count toward
     the lift sum.
   - The 3 8.4a budget exceptions (q2182, q2206, q2207) are
     present in Section 8.1's full 66-row inventory but **filtered
     out of `pursuable_targets`**. They are NOT subtracted from
     the 167-question denominator; the headline target stays
     151/167. 8.4a entries become permanent non-A rows that count
     against the 151 ceiling.
   - The required lift rate of `pursuable_targets` is
     (151 - current A) / `len(pursuable_targets)`. With May-5
     numbers: 50 / 63 = 79.4%.
...
pursuable set; Stream 26.0 / 26.1 / 26.2 / 26.3 / 26.4 / 26.5 each
have target candidate sets and lift estimates; Section 7 has a
12-day schedule; Section 9 has the correct denominator math
(50 / 63 = 79.4% required lift rate of `pursuable_targets`).

The next action is **start Stream 26.0 regression recovery on day 1**
per the Section 7 schedule. The 30-row audit bucket (Stream 26.3d)
and the 19-row regression list (Stream 26.0) need per-question
