...
commit, hash-recording follow-ups, a round-2 revise commit that addressed the
round-1 implementation-audit feedback on `connectionTimeoutMillis`, and this
round-3 revise commit that replaces the `Promise.race` probe with a real
`pg.Client`-level `connectionTimeoutMillis: 2000`.
...
1. **`createPool()` body is byte-for-byte unchanged; probe gets `connectionTimeoutMillis: 2000` via a transient `pg.Client` (revised after round-3 implementation audit).** Round 1 added `connectionTimeoutMillis: 2_000` to the three `new Pool({...})` literals; round-2 audit flagged that as a behavior change to the opt-out and production paths, so round 2 reverted to byte-for-byte original literals and used `Promise.race([target.query("SELECT 1"), setTimeout(2000)])` for the probe budget. Round-3 audit then flagged that approach because the slice Goal, Required services / env, and Step 3 explicitly call for a `connectionTimeoutMillis`-bounded probe — `Promise.race` only stops awaiting after 2 s while the underlying `pg` connect attempt can continue. Round 3 (this commit) replaces `Promise.race` with a transient `pg.Client` constructed from `pool.options` plus `connectionTimeoutMillis: 2_000`, then `await probeClient.connect()` + `await probeClient.query("SELECT 1")` + `probeClient.end()`. This applies pg's native connect-timeout machinery to the probe at the network layer, leaves the singleton `pool` byte-for-byte untouched (opt-out/prod still use pg's default no-client-side-connect-timeout behavior), and satisfies the slice contract that the probe carries `connectionTimeoutMillis: 2 s`. The pool singleton's URL → `NEON_DB_HOST` → `DB_*`-with-defaults ladder, SSL handling, and `statement_timeout` semantics remain unchanged.
...
**Self-check — gate exit codes (all run from `web/`, round-3 revise):**
