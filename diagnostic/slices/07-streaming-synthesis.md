...
- **Scope of UI gate (resolves round-4 Medium).** The deterministic UI gate (`streaming-ui.test.mjs`) covers the contract of the `consumeChatStream` helper exported from `web/src/components/chat/streamingConsumer.ts`: progressive `onDelta(cumulativeText, chunkText)` invocations for each SSE delta frame, and exactly one `onFinal` invocation with the embedded `ChatApiResponse`. It does **NOT** exercise `ChatWorkspace.tsx`'s React state-update wiring at runtime — doing so would require a React renderer (`@testing-library/react` or `react-test-renderer`) that is not currently in `web/package.json` and is outside this slice's scope. To keep the wiring's blast radius minimal, the `onDelta` callback inside `ChatWorkspace.tsx` is constrained to a single mechanical line that forwards `cumulativeText` straight to `setMessages` (see Step 3 wording). TypeScript catches signature mismatches at `npm run typecheck`, and the wiring is short enough to verify by code review. **The slice therefore does not claim a runtime guarantee that `ChatWorkspace.tsx` progressively re-renders streamed assistant text** — it claims only that (a) the SSE producer (`route.ts`) emits ≥ 3 delta frames whose concatenation equals the final answer, and (b) the consumer helper (`streamingConsumer.ts`) invokes `onDelta` with cumulative text on each delta frame. Progressive DOM rendering follows from React's standard re-render-on-setState semantics applied to that callback contract; it is not separately gated.
...
- [ ] `streaming-ui.test.mjs` asserts that the `consumeChatStream` helper exported from `web/src/components/chat/streamingConsumer.ts` invokes its `onDelta(cumulativeText, chunkText)` callback with the cumulative concatenation of delta texts after each delta frame arrives (not only after the stream closes) and invokes `onFinal` exactly once with the embedded `ChatApiResponse` payload. Deterministic gate over the helper's callback contract only; no manual verification of the helper. **Scope note (round-4 Medium):** this gate does NOT exercise `ChatWorkspace.tsx`'s React state-update wiring at runtime — the slice does not claim a runtime guarantee that the rendered DOM progressively updates. The wiring in `ChatWorkspace.tsx` is constrained by Step 3 to a single mechanical `setMessages(prev => withAssistantText(prev, cumulativeText))`-shape line, verified by `npm run typecheck` (callback signature) and code review. See Decisions § Scope of UI gate.
