...
- Scope diff -> **PASS**. `git diff --name-only integration/perf-roadmap...HEAD` stays within `diagnostic/_state.md`, `diagnostic/slices/07-zero-llm-path-tighten.md`, and the declared `## Changed files expected` paths.
- Acceptance: cold deterministic, production-mode, zero LLM -> **PASS**. Deterministic branch bypasses `cachedSynthesize` at `web/src/app/api/chat/route.ts:727`; all 32 `DETERMINISTIC_KEYS` assert Anthropic counter `0` at `web/scripts/tests/zero-llm-path.test.mjs:342`.
- Acceptance: warm answer-cache-hit, zero LLM -> **PASS**. Cache-hit return remains ahead of LLM call sites at `web/src/app/api/chat/route.ts:473`; repeated deterministic request asserts counter `0` at `web/scripts/tests/zero-llm-path.test.mjs:379`.
- Acceptance: LLM-required negative control -> **PASS**. Non-template path still reaches `generateSqlWithAnthropic` at `web/src/app/api/chat/route.ts:416`; test asserts counter `> 0` at `web/scripts/tests/zero-llm-path.test.mjs:414`.
- Acceptance: dev-only assertion verified -> **PASS**. Helper throws for all three call sites at `web/src/lib/zeroLlmGuard.ts:1`; direct unit test covers `generateSqlWithAnthropic`, `repairSqlWithAnthropic`, and `cachedSynthesize` at `web/scripts/tests/zero-llm-path.test.mjs:449`.
- Acceptance: non-deterministic no-throw -> **PASS**. Guard is scoped to `generationSource === "deterministic_template"` at `web/src/lib/zeroLlmGuard.ts:9`; no-throw test is at `web/scripts/tests/zero-llm-path.test.mjs:484`.
- Acceptance: production no-throw -> **PASS**. Production early return is at `web/src/lib/zeroLlmGuard.ts:6`; no-throw test is at `web/scripts/tests/zero-llm-path.test.mjs:504`.
- Decision -> **PASS**.
