...
# Risk-section grep: legacy field names must not appear outside the
# files this slice deliberately edits and the slice file itself.
# (Carved out: docs/prompt_outcomes_summary.md is updated in step 3, so
# this grep should return zero hits in source/test/fixture/artifact paths.)
! git grep -nE 'answer_grade|semantic_conformance_grade|answerGradeCounts|semanticConformanceGradeCounts' \
  -- ':!diagnostic/slices/11-multi-axis-grader-redesign.md' ':!scripts/loop/test_grading_gate.sh' \
