-- Canonical location: sql/migrations/deploy/007_semantic_summary_contracts.sql
...
-- Grid/finish reconciliation contract with explicit source provenance.
...
-- Stint-level summary contract backed by semantic lap data.
...
-- Driver-level strategy summary contract.
...
-- Driver/session level pace + strategy + classification summary.
CREATE OR REPLACE VIEW core.driver_session_summary AS
WITH lap_rollup AS (
  SELECT
    le.session_key,
    le.meeting_key,
...
SELECT
  ...
  gvf.finish_source
FROM lap_rollup lr
LEFT JOIN core.strategy_summary ss
...
-- Race progression contract at lap grain (race sessions only).
...
-- Canonical pit-cycle contract at driver/pit-event grain.
-- This view provides explicit evidence sufficiency flags so downstream synthesis can avoid over-claiming.
...
-- Canonical undercut/overcut evidence contract.
-- One row per driver pit event, paired to the nearest rival pit event in the same session.
CREATE OR REPLACE VIEW core.strategy_evidence_summary AS
WITH pit_cycle AS (
  SELECT *
  FROM core.pit_cycle_summary
),
rival_candidates AS (
  SELECT
    a.session_key,
...
-- Optional lap phase contract for phase-aware pace analytics.
...
-- Optional lap-number context summary for relative pace reasoning.
...
-- Optional telemetry-to-lap bridge with aggregated lap-window telemetry.
...
-- Metric registry backfill for summary-layer metrics.
...
VALUES
  (
    ...
    'session,driver',
    'stable',
    'Average lap duration across policy-valid laps.',
    'core.driver_session_summary',
    'avg_valid_lap',
    'AVG(lap_duration) FILTER (WHERE is_valid)'
  ),
  (
    'pit_stop_count',
    'Pit Stop Count',
    ...
    'session,driver',
    'stable',
    'Count of pit stops inferred from stint boundaries.',
    'core.strategy_summary',
    'pit_stop_count',
    'GREATEST(total_stints - 1, 0)'
  ),
  (
    'positions_gained',
    'Positions Gained',
    ...
    'session,driver',
    'stable',
    'Grid minus finishing position using official/fallback sources.',
    'core.grid_vs_finish',
    'positions_gained',
  ...
  (
    'degradation_per_lap',
    'Stint Degradation Per Lap',
    'strategy',
    'core.stint_summary',
    'session,driver,stint',
    'stable',
    'Linear degradation slope of lap duration vs tyre age within stint.',
    'core.stint_summary',
    'degradation_per_lap',
    'REGR_SLOPE(lap_duration, tyre_age_on_lap)'
  )
