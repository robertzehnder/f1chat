-- Deploy openf1:048_analytics_corner_analysis to pg
-- requires: 047_core_sample_lap_position
--
-- Phase 26.2a (slice 21-corner-analysis): per-(session, driver,
-- lap, corner_id) entry / apex / exit speeds. Buckets raw.car_data
-- speed samples into f1.track_segments corner zones using the
-- time_fraction approximation from slice 047 (samples whose
-- in-lap time_fraction falls within the corner's start_normalized
-- to end_normalized range).
--
-- Schema:
--   entry_speed_kph     = MAX(speed) in the entry sub-window
--                         [start_normalized - 0.01, start_normalized + 0.005]
--   apex_min_speed_kph  = MIN(speed) inside the corner zone
--                         [start_normalized, end_normalized]
--   exit_speed_kph      = MAX(speed) in the exit sub-window
--                         [end_normalized - 0.005, end_normalized + 0.01]
--
-- Time-fraction approximation note: see slice 047 comment. Corner
-- zones in track_segments are spatially-normalized (lap-distance
-- fractions); time_fraction is conservative (overestimates corner
-- sample counts) but doesn't miss samples. For first-cut entry/
-- apex/exit speeds at named corners, this is sufficient.
--
-- Cost mitigation: the matview filters to Race + Qualifying +
-- Sprint + Sprint Qualifying sessions only (analytics is for
-- competitive sessions). Practice sessions are excluded to keep
-- the build budget within Neon's per-query limit.
