-- Deploy openf1:054_driver_score_grid_source to pg
-- requires: 053_chat_persistence
--
-- The OpenF1 starting_grid endpoint attaches grid rows to the
-- QUALIFYING session that set the grid, not the Race session — so
-- 045's qual_axis_raw join (session_name = 'Race') could never match
-- once real grid data landed (2026-08 backfill). Rebuild
-- driver_performance_score_data reading grids from the Qualifying
-- session (which sets the race grid). Definition otherwise verbatim
-- from 052's rebuild (045 with year >= 2025).
