-- Revert openf1:025_alias_view_diacritic_alignment from pg
--
-- Re-runs the original sql/005_helper_tables.sql view + index
-- definitions (LOWER(BTRIM(...)) without f1_unaccent wrapping) so
-- the schema returns to its pre-migration shape. The CREATE OR
-- REPLACE VIEW + DROP/CREATE INDEX pattern matches what 005 does.

BEGIN;

CREATE OR REPLACE VIEW core.session_search_lookup AS
WITH base_sessions AS (
  SELECT s.session_key, s.meeting_key, s.year, s.session_name, s.session_type,
...
intrinsic_aliases AS (
  ...
  FROM base_sessions bs WHERE bs.country_name IS NOT NULL AND BTRIM(bs.country_name) <> ''
  UNION ALL
  SELECT bs.*, bs.location AS alias_text,
         LOWER(BTRIM(bs.location)) AS normalized_alias,
         'location'::TEXT, 'intrinsic'::TEXT
  FROM base_sessions bs WHERE bs.location IS NOT NULL AND BTRIM(bs.location) <> ''
  UNION ALL
  ...
  FROM base_sessions bs WHERE bs.circuit_short_name IS NOT NULL AND BTRIM(bs.circuit_short_name) <> ''
  UNION ALL
  SELECT bs.*, bs.session_name AS alias_text,
         LOWER(BTRIM(bs.session_name)) AS normalized_alias,
         'session_name'::TEXT, 'intrinsic'::TEXT
  FROM base_sessions bs WHERE bs.session_name IS NOT NULL AND BTRIM(bs.session_name) <> ''
),
venue_aliases AS (
...
WITH latest_identity AS (
  ...
  FROM raw.drivers d LEFT JOIN raw.sessions s ON s.session_key = d.session_key
...
all_aliases AS (
  SELECT * FROM derived_aliases UNION ALL SELECT * FROM seed_aliases
)
...
FROM all_aliases aa LEFT JOIN driver_meta dm ON dm.driver_number = aa.driver_number
...
WITH seed_aliases AS (
  SELECT NULLIF(BTRIM(l.alias_text), '') AS alias_text,
...
observed_aliases AS (
  SELECT NULLIF(BTRIM(d.team_name), '') AS alias_text,
...
all_aliases AS (
  SELECT * FROM seed_aliases UNION ALL SELECT * FROM observed_aliases
)
SELECT DISTINCT ON (
...
CREATE UNIQUE INDEX uq_session_venue_alias_lookup
  ON core.session_venue_alias_lookup(
    COALESCE(normalized_alias, LOWER(BTRIM(alias_text))),
