...
try {
  const cols = await pool.query(`SELECT column_name FROM information_schema.columns WHERE table_schema='raw' AND table_name='intervals' ORDER BY ordinal_position`);
  console.log("raw.intervals columns:", cols.rows.map(r=>r.column_name).join(", "));
  const sample = await pool.query(`SELECT * FROM raw.intervals WHERE session_key=9839 ORDER BY date LIMIT 5`);
